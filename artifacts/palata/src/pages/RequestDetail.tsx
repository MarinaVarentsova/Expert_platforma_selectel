...
const ORDER_STATUS: Record<string, { label: string; cls: string }> = {
  ...
  expert_selected_by_customer: { label: "Эксперт выбран",  cls: "bg-[#0F4C9A]/10 text-[#002B5C]" },
  ...
  if (role === "expert" && myLosingMatch) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-base font-semibold text-slate-700 mb-1">Информация по данному заказу недоступна</p>
        <p className="text-sm text-slate-400">Просмотр заказа закрыт</p>
      </div>
    );
  }
                ...
                return (
                  <div key={m.id} className="rounded-lg border border-slate-200 overflow-hidden">
                    ...
                    <div className="px-4 py-3 bg-slate-50 flex items-center justify-between gap-3">
                      ...
                      {/* Hide "Предложено" for customer when not yet selected; show it when customer already picked this expert */}
                      {(role !== "customer" || m.status !== "proposed" || !!m.responded_at) && (
