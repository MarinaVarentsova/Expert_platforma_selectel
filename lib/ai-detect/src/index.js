...
{"detected": false, "direction_name": null, "confidence": 0, "reason": "причина", "matched_markers": []}`;
  ...
  if (!gatewayUrl) {
    console.log("[AI-PROD] AI_GATEWAY_URL not set — no fallback, returning error");
    return {
      status: "gateway_error",
      httpStatus: 0,
      errText: "AI_GATEWAY_URL_MISSING",
    };
  }
  ...
  console.log("[AI-PROD] Requested model =", model);
  ...
  const httpStatus = gatewayResponse.status;
  const responseBody = await gatewayResponse.text().catch(() => "");

  console.log("[AI-PROD] AI Gateway response.status =", gatewayResponse.status);
  console.log("[AI-PROD] AI Gateway response.statusText =", gatewayResponse.statusText);
  console.log(
    "[AI-PROD] AI Gateway response.headers =",
    Object.fromEntries(gatewayResponse.headers.entries())
  );

  if (!gatewayResponse.ok) {
    console.log("[AI-PROD] AI Gateway response.body =", responseBody);
    return {
      status: "gateway_error",
      httpStatus,
  ...
  //   1. choices-wrapper: { choices: [{ message: { content: "<json-string>" } }] }
  //   2. Direct (Yandex and others): { detected: true, direction_name: "...", ... }

  let gatewayData;
  try {
    gatewayData = JSON.parse(responseBody);
  } catch {
    return { status: "parse_error", httpStatus, rawContent: responseBody.slice(0, 500) };
  }

  let parsed;
  if (gatewayData.choices?.[0]?.message?.content !== undefined) {
    // choices-wrapper format — content is a JSON string
