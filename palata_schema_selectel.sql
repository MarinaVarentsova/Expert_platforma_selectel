-- palata_schema_selectel.sql
-- Подготовлено для PostgreSQL (Selectel).
-- Удалено: auth.*, POLICY, ROW LEVEL SECURITY, GRANT/REVOKE, ALTER OWNER,
--          \restrict/\unrestrict, pg_dump-комментарии, FK на auth.users.
--
-- ВАЖНО — в исходном дампе отсутствуют определения пользовательских типов.
-- Перед запуском этого скрипта создайте типы вручную:
--
--   CREATE TYPE public.palata_user_role AS ENUM ('customer', 'expert', 'admin');
--   CREATE TYPE public.palata_order_status AS ENUM ('draft', 'published', 'matching', 'in_progress', 'completed', 'cancelled');
--   CREATE TYPE public.palata_expert_profile_status AS ENUM ('draft', 'pending_review', 'active', 'suspended');
--   CREATE TYPE public.palata_match_status AS ENUM ('proposed', 'contacts_opened', 'can_start_from', 'accepted', 'accepted_work', 'declined', 'closed_by_other_expert', 'withdrawn', 'completed', 'customer_declined_start_date');
--   CREATE TYPE public.palata_decline_reason AS ENUM ('not_competent', 'busy', 'region_mismatch', 'other');
--
-- ВАЖНО — в исходном дампе отсутствуют определения функций, используемых триггерами.
-- Перед запуском создайте функции:
--   public.palata_set_updated_at()
--   public.palata_refresh_expert_stats_on_match()
--   public.palata_refresh_expert_stats()
--
-- ВАЖНО — внешний ключ palata_users_id_fkey (ссылался на auth.users) удалён.
-- В автономном PostgreSQL palata_users.id является самостоятельным первичным ключом.
--
