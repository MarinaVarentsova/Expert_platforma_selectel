...
async function handleAdminDeleteUser(req, res) {
  ...
  // ── 3. Resolve user and role ───────────────────────────────────────────────
  const userRow = (await pool.query(
    `SELECT id, role, is_active FROM public.palata_users WHERE id = $1 LIMIT 1`,
    [userId],
  )).rows[0];
  ...
  if (role === "admin") {
    return res.status(422).json({
      ...
      message: "Удаление данных администратора не разрешено через этот endpoint",
    });
  }

  // ── 4. Collect summary (always — for dry_run response and live audit) ──────
  ...
  try {
    ...
    await client.query("ROLLBACK").catch(() => {});
    console.error("[ADMIN-DELETE-USER] transaction failed", { stack: err.stack });
    return res.status(500).json({ success: false, error: "ANONYMIZE_FAILED", message: err.message });
  } finally {
    client.release();
  }
...
async function _writeAuditLog(pool, adminId, targetUserId, role, dryRun, counts, fileResults) {
  ...
  const details = {
    ...
    ...(filesNotDeleted.length > 0 && {
      note: "Некоторые файлы не удалены физически. Удалите их вручную из Selectel Object Storage.",
    }),
  };
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO public.palata_admin_audit_log
         (action, initiated_by, target_user_id, details)
       VALUES ($1, $2, $3, $4)`,
      ["anonymize_user", adminId, targetUserId, JSON.stringify(details)],
    );
  } catch (auditErr) {
    // Never let audit failure break the main operation
    console.error("[ADMIN-DELETE-USER] audit log write failed", { stack: auditErr.stack });
  }
}

function _buildSuccessResponse(res, userId, role, counts, fileResults) {
  const filesDeleted    = fileResults.filter(f => f.deleted).length;
  const filesNotDeleted = fileResults.filter(f => !f.deleted);
  ...
  return res.json({
    ...
    files: {
      ...
      not_deleted: filesNotDeleted.map(f => ({
        path:   f.bucketPath,
        reason: f.reason,
        ...(f.detail && { detail: f.detail }),
      })),
      ...(filesNotDeleted.length > 0 && {
        note: "Не удалось удалить физические файлы. Удалите их вручную из Selectel Object Storage.",
      }),
    },
  });
}

app.post("/api/palata/admin/users/:userId/delete", (req, res) => {
  handleAdminDeleteUser(req, res).catch(err => {
