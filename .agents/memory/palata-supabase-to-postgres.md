...
**How to apply:** When scoping a partial migration, migrate shared plumbing operations that are already role-agnostic (same SQL regardless of role) if they're on the requested endpoint list, but never touch role-exclusive tables/branches outside scope. New backend endpoints for unauthenticated registration flows (pre-login) don't need `requireAdmin()`-style gating.

**Migration status (as of 2026-07-11):**
- Done: `palata_customer_profiles`, `palata_expert_profiles`, `palata_expert_regions`, `palata_expert_directions`, `palata_expert_certificates`
- Remaining on Supabase: `Register.tsx` cert insert (technical debt — entire file is explicitly out of scope), `palata_users`, `palata_expertise_directions` select, and all other tables not yet explicitly migrated.
- `palata_expert_certificates` endpoints: GET list (filters: expert_ids, status, direction_id, valid_from, valid_to, limit), GET /:expertId, POST (bearer, replace-all pattern). cert_direction_ids is uuid[] — INSERT uses `$n::uuid[]` cast; filter uses `cert_direction_ids @> ARRAY[$n::uuid]`.
